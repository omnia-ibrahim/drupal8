...
/**
 * Provides a form for reverting a hi revision.
 *
 * @ingroup dino_roar
 */
class fooRevisionRevertForm extends ConfirmFormBase {
   ...
   * The hi revision.
   *
   * @var \Drupal\dino_roar\Entity\fooInterface
   */
  protected $revision;
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('hi: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    drupal_set_message(t('hi %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\dino_roar\Entity\fooInterface $revision
